...
void execute_instruction(char** instruction, contexto_ejecucion* ce){
    ...
    switch(keyfromstring(instruction[0])){
        ...
        case I_MOV_IN: //MOV_IN (Registro, Dirección Lógica)
            ...
            if(sigsegv != 1){
                ...
                log_info(cpu_logger, "PID: %d - Acción: LEER - Segmento: %d - Dirección Fisica: %d",
                        ce->id,id_segmento, direccion_fisica);
                    log_info(cpu_logger,"YA GUARDE VALOR EN REGISTRO %s!!",value);
            }else{
                log_error(cpu_logger,"PID: <%d> - Error SEG_FAULT- Segmento: <%d> -Offset: <%d> - Tamaño: <%d>",ce->id,id_segmento_con_segfault,desplazamiento_segfault,tamanio_segfault);
                enviar_ce(socket_kernel, ce, SEG_FAULT, cpu_logger);
                sigsegv = 0;
        ...
        case I_MOV_OUT: ///MOV_OUT (Dirección Lógica, Registro):
            ...
            log_warning(cpu_logger,"LA INFO DEL SEGMENTO GLOBAL ES: %d id, %d BASE, %d tamanio",segmento_global->id_segmento,segmento_global->direccion_base,segmento_global->tamanio_segmento);
                char* register_value_mov_out = encontrarValorDeRegistro(register_mov_out);

                log_warning(cpu_logger,"LA INFO DEL SEGMENTO GLOBAL ES: %d id, %d BASE, %d tamanio",segmento_global->id_segmento,segmento_global->direccion_base,segmento_global->tamanio_segmento);
              

                log_warning(cpu_logger,"encontre valor registro: %s",register_value_mov_out);
                ...
                log_warning(cpu_logger,"ya escribi valor");
                int code_op = recibir_operacion(conexion_cpu); // Si ta todo ok prosigo, si no ta todo ok que hago?
                log_warning(cpu_logger,"recibo codop :%d",code_op);
                if(code_op == MOV_OUT_OK) {  
                    log_warning(cpu_logger,"entre en em if del movOutOk");
...
int traducir_direccion_logica(int logical_address, contexto_ejecucion* ce, int valor_a_sumar) {
    ...
    log_error(cpu_logger,"desplazamiento segmento:%d",desplazamiento_segmento);
    segmento_global = list_get(segment_table_ce, num_segmento);
